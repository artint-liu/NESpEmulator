...
namespace {
  ...
  const std::unordered_map<int, PixelEngine::KeyStatus> statusMap{
      {WM_KEYDOWN, PixelEngine::KeyStatus::Press},
      {WM_KEYUP, PixelEngine::KeyStatus::Release},
      {WM_CHAR, PixelEngine::KeyStatus::Repeat},
  };
...
PixelEngine::PixelEngine(int width, int height, std::string_view title, int scale)
  ...
  g_pScreen = new Gdiplus::Bitmap(width, height, PixelFormat32bppRGB);
...
{
  onBegin();
  m_pThread = new std::thread{ &PixelEngine::userThread, this };
}
...
void PixelEngine::render() {
  ...
  if (s == Gdiplus::Ok) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++, index++, pColor++, pSrc++) {
        DWORD d = *pSrc;
...
void PixelEngine::keyCallback(PixelEngine* pixelEngine, int key, int action)
{
